---
title: "Lecture 2: Summarizing, Tidying and Visualizing Data"
...
- To get a sense of the **distribution** of a [continuous variables]{.hi_purple}: plot a **histogram**
  - Group values into *bins* and plot the number of observations that fall into each bin
...
The mean, $\bar{x}$, is simply the sum of all the countries' hours of worked $(x_i)$ divided by the number of countries $(N)$:
...
- The [mean]{.hi_purple} of a binary variable is equal the **percentage of 1s**:
...
## Covariance and correlation {#cov_cor .pause}
...
- Difficult to interpret because sensitive to the variables' dispersions from the mean $\rightarrow$ [correlation]{.hi_blue}

. . .

- The [correlation]{.hi_blue} is a measure of the strength of the **linear association** between two variables:
...
- Correlation is [always between -1 and 1!]{.hi_purple} ([proof]())
...
3. Compute the three quartiles of the distribution of `hours_worker`. What's the 75th percentile?
...
* The `tidyverse` is a collection of packages that facilitate data [manipulation]{.hi_purple} and [visualization]{.hi_blue}.
...
## `group_by()` and `mutate` {#group_by .pause}
...
- [Cédric Scherer](https://www.cedricscherer.com/)'s wonderful [*Graphic Design with ggplot2*](https://rstudio-conf-2022.github.io/ggplot2-graphic-design/) tutorial
...
3. A `scatter plot` of hours per female worker (y-axis) with respect to hours per male worker (x-axis). Once you've created the scatter plot, within the appropriate geom_* set: size to 2, alpha to 0.25, colour to "#d90502". Add labels (labs) to the plot so that it is cleaner.
